...
class DrawableWrapperGingerbread extends Drawable implements Drawable$Callback, DrawableWrapper, TintAwareDrawable
{
    ...
    DrawableWrapperGingerbread(@Nullable final Drawable wrappedDrawable) {
        this.mState = this.mutateConstantState();
        this.setWrappedDrawable(wrappedDrawable);
    }
    
    DrawableWrapperGingerbread(@NonNull final DrawableWrapperState mState, @Nullable final Resources resources) {
        this.mState = mState;
    ...
    private boolean updateTint(final int[] array) {
        final boolean compatTintEnabled = this.isCompatTintEnabled();
        boolean b = false;
        if (compatTintEnabled) {
            ...
            if (mTint != null && mTintMode != null) {
                ...
                if (this.mColorFilterSet && colorForState == this.mCurrentColor) {
                    final PorterDuff$Mode mCurrentMode = this.mCurrentMode;
                    b = false;
                    if (mTintMode == mCurrentMode) {
                        return b;
                    }
                }
    ...
    private static class DrawableWrapperStateBase extends DrawableWrapperState
    {
        ...
        public Drawable newDrawable(@Nullable final Resources resources) {
            return new DrawableWrapperGingerbread((DrawableWrapperState)this, resources);
        }
    }
}
